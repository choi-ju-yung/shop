	...
	<div class="cl-wrap">
								...
								<c:choose>
									<c:when test="${not empty room.productImg}">
										<img class="cl-product-thumb"
											...
											alt="상품" onerror="this.outerHTML='<div class=\'cl-product-thumb-placeholder\'>
		...
		<div class="cl-body">
			<div class="cl-top-row">
				<span class="cl-partner"> <c:out
						value="${not empty room.otherUserName ? room.otherUserName : '상대방'}" />
				</span>
				<span class="cl-time" data-ts="${room.lastMessageTime}"></span>
			</div>
			<span class="cl-last-msg"> ${empty room.lastMessage ? '메시지 없음' : room.lastMessage}
			</span>
		</div>
	...
	document
			.addEventListener(
					...
					function() {
						...
						document.querySelectorAll('.cl-time[data-ts]').forEach(
								function(el) {
									el.textContent = fmtTime(el
						...
						stompClient.connect({}, function() {
							stompClient.subscribe('/topic/chat-list/' + userNo,
									function(message) {
										updateChatListItem(JSON
												.parse(message.body));
									});
						});

						window.openChatRoom = function(productId, targetUserNo,
								roomId) {
							var item = document.getElementById('chat-item-'
									+ roomId);
							if (item) {
							...
							if (stompClient && stompClient.connected) {
								stompClient.send('/app/chat/read', {}, JSON
										.stringify({
											roomId : roomId,
											userNo : userNo,
							...
							const popup = window.open('',
									'chatPopup_' + roomId,
									'width=460,height=660,resizable=yes');
						...
						function buildItemHtml(data, existingItem) {
							...
							if (existingItem) {
								var thumb = existingItem
										.querySelector('.cl-product-thumb');
								if (thumb)
									productThumbHtml = '<img class="cl-product-thumb" src="' + thumb.src + '" alt="상품" onerror="this.style.display=\'none\'">';
								var profileMini = existingItem
										.querySelector('.cl-profile-mini');
								if (profileMini)
									profileMiniHtml = profileMini.innerHTML;
								if (!partnerName) {
									var partnerEl = existingItem
											.querySelector('.cl-partner');
									if (partnerEl)
										partnerName = partnerEl.textContent
												.trim();
								}
							}
							if (!partnerName)
							...
							if (data.unreadCount === -2) {
								var curBadge = existingItem ? existingItem
										.querySelector('.cl-unread') : null;
								displayCount = curBadge ? (parseInt(curBadge.textContent) || 0)
										: 0;
							} else {
								displayCount = isPopupOpen(String(data.roomId)) ? 0
										: data.unreadCount;
							}
							if (displayCount > 0)
								unreadHtml = '<span class="cl-unread">'
										+ displayCount + '</span>';

							return '<div class="cl-media">' + productThumbHtml
									+ '<div class="cl-profile-mini">'
									+ profileMiniHtml + '</div>' + '</div>'
									+ '<div class="cl-body">'
									+ '<div class="cl-top-row">'
									+ '<span class="cl-partner">' + partnerName
									+ '</span>'
									+ '<span class="cl-time">'
									+ fmtTime(new Date().toISOString())
									+ '</span>' + '</div>'
									+ '<span class="cl-last-msg">'
									+ (data.lastMessage || '') + '</span>'
									+ '</div>' + '<div class="cl-badge-area">'
									+ unreadHtml + '</div>';
						}

						function updateChatListItem(data) {
							const container = document
									.getElementById('chat-list-container');
							const existingItem = document
									.getElementById('chat-item-' + data.roomId);

							/* 메시지 없음 → 뱃지만 갱신 */
							if (!data.lastMessage) {
								if (existingItem) {
									var badgeArea = existingItem
											.querySelector('.cl-badge-area');
									if (badgeArea) {
										badgeArea.innerHTML = '';
										if (data.unreadCount > 0
												&& !isPopupOpen(String(data.roomId))) {
											badgeArea.innerHTML = '<span class="cl-unread">'
													+ data.unreadCount
													+ '</span>';
										}
									}
